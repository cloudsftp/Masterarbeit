        ...
        not any(
            [
                is_outdated(
                    ...
                    frame.diagram.model.model_source_file_path,
                )
...
def get_num_cores(frame: frame.Frame) -> int:
    ...
    if not res:
        raise CustomException(
            f"No number of cores configured for host {host}, please specify with option --num-cores"
        )
...
def get_num_points(frame: frame.Frame) -> int:
    ...
    if len(frame.scan) == 2:
        ...
        raise CustomException(
            f"get_num_points not implemented for {len(frame.scan)} scans"
        )
        ...
        while True:
            ...
            if output:
                ...
                if progress_indicator_match:
                    ...
                    print(100 * " ", end="\r")
                    print(
                        f"Progress: {int(progress)}%, ETA: {eta_str}",
                        end="\r",
                        flush=True,
                    )
...
def get_data_file_paths(frame: execution.frame.Frame) -> List[Path]:
    ...
    elif frame.diagram.type == DiagramType.BIFURCATION_MULTICOLOR:
        return [get_cyclic_bif_path(frame)]
    else:
        raise CustomException(
            f"Executing simulation for type {frame.diagram.type} not yet supported!"
        )
