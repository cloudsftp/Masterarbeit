...
def invert_parameter_ranges(
    ranges: List[ParameterRange], *indices: int
) -> List[ParameterRange]:
    res = deepcopy(ranges)
...
def load_parameter_range_specification_from_dict(
    ...
    if not isinstance(name, str):
        raise CustomException(
            '"name" in diagram parameter range specification should be str'
        )
    ...
    if not (isinstance(start, float) or isinstance(start, int)):
        raise CustomException(
            '"start" in diagram parameter range specification should be float or int'
        )
    ...
    if not (isinstance(stop, float) or isinstance(stop, int)):
        raise CustomException(
            '"stop" in diagram parameter range specification should be float or int'
        )
...
def load_parameter_range_from_dict(
    ...
    if "resolution" not in config:
        raise CustomException(
            '"resolution" not in diagram parameter range configuration'
        )

    resolution = config["resolution"]
    if not isinstance(resolution, int):
        raise CustomException(
            '"resolution" in diagram parameter range configuraion should be int'
        )
    ...
    if "parameters" not in config:
        raise CustomException(
            '"parameters" not in diagram parameter range configuration'
        )

    parameters = config["parameters"]
    if not isinstance(parameters, list):
        raise CustomException(
            '"parameters" in diagram parameter range configuration should be list'
        )
...
def load_diagram_from_dict(
    ...
    elif type == "bifurcation multicolor":
        obj.type = DiagramType.BIFURCATION_MULTICOLOR
    else:
    ...
    if "parameters" not in config or not config["parameters"]:
        ...
        if not isinstance(parameters, dict):
            raise CustomException(
                '"parameters" in diagram configuration should be dict'
            )

        for name in parameters:
            if not (
                isinstance(parameters[name], float) or isinstance(parameters[name], int)
            ):
                raise CustomException(
                    f'Parameter "{name}" in diagram configuration file should be float or int'
                )
