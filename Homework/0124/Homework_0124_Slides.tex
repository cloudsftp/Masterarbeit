...
\begin{frame}{(i.b) $a_L = -1, a_R = 0.5$}
    \begin{itemize}
        \item[$\mu >= 0$] Stable fixpoint at $2 \mu$, \\
        \item[$\mu < 0$] \begin{itemize}
            \item Infinite unstable 2-cycles in $(\mu, 0)$
            \item Unstable fixed point at $\frac{\mu}{2}$
        \end{itemize}
    \end{itemize}
...
\begin{frame}{(i.c) $a_L = a_R = -1$ (Not a bifurcation)}
    \begin{itemize}
        \item Infinite unstable 2-cycles everywhere 
        \item Unstable fixed point at $\frac{\mu}{2}$
    \end{itemize}
\end{frame}
    ...
    \begin{figure}
        \centering
            \forloop{n}{0}{\value{n} < 5}{
                \setcounter{f}{\value{n}+1}
                \includegraphics<\arabic{f}>[width=0.6\textwidth]{
                    Cob_id/Autogen/Frame_00\twodigits{\value{n}}/result.png
                }
            }
        \caption{Cobwebs $\mu \in \{0.02, 0.01, 0, -0.01, -0.02\}$}
    \end{figure}
...
\begin{frame}{(0.a) Fixing $a_L = -2, a_R = 0.75$}
    \begin{enumerate}
        \item[$\mu >= 0$] Stable fixed point at $4 \mu$ w/ absorbing interval $(-\infty, \infty)$
        \item[$\mu < 0$] Unstable fixed point at $\frac{\mu}{3}$
    \end{enumerate}
\end{frame}
...
\begin{frame}{(0.b) Fixing $a_L = -2, a_R = 0.5$}
    \begin{itemize}
        \item[$\mu >= 0$] Stable fixed point at $2 \mu$
        \item[$\mu < 0$] \begin{itemize}
            \item Infinite unstable 4-cycles in $(2 \mu, -\mu)$
            \item Unstable 2-cycle at $\frac{3 \mu}{4}$ and $\frac{-\mu}{2}$
            \item Unstable fixed point at $\frac{\mu}{3}$
        \end{itemize}
    \end{itemize}
    Absorbing interval: $(-\infty, \infty)$
\end{frame}
...
\begin{frame}{(0.c) Fixing $a_L = -2, a_R = 0$}
    \begin{enumerate}
        \item[$\mu >= 0$] Stable fixed point at $\mu$
        \item[$\mu < 0$] \begin{itemize}
            \item Stable 2-cycle at $-\mu$ and $\mu$
            \item Unstable fixed point at $\frac{\mu}{3}$
        \end{itemize}
    \end{enumerate}
    Absorbing interval: $(-\infty, \infty)$
...
\begin{frame}{(ii) What happens if we change $a_R$?}
    \begin{figure}
        \centering
        \subfloat[Initial State $-\epsilon$]{
            \includegraphics<1>[width=0.4\textwidth]{2D_ii_aR/result.png}
            %\includegraphics<2>[width=0.4\textwidth]{2D_ii_aL_Zoomed/result.png}
        } \qquad
        \subfloat[Initial State $\epsilon$]{
            \includegraphics<1>[width=0.4\textwidth]{2D_ii_aRp/result.png}
            %\includegraphics<2>[width=0.4\textwidth]{2D_ii_aLp_Zoomed/result.png}
        }
        \caption{Varying $a_L$}
    \end{figure}
