...
\hl{
	...
	We can see this in \Cref{fig:state.og.overlapping.chains}, it shows the boundaries of parameter regions with the same period.
	...
	For \Cref{fig:state.og.overlapping.chains.full}, the fixed and varied parameters are the same as in previous 2D scans like \Cref{fig:setup.og.dynamics.period}.
...
\footnotetext[1]{
	\hl{
		Computationally, we don't actually use the symbolic sequences but employ a trick.
		This trick involves taking advantage of the symmetry and halving the model.
		It will be used in later sections of \Cref{chap:setup} and explained in-depth in \Cref{chap:add}.
	}
	\todo{Make sure this footnote is placed correctly}
}
