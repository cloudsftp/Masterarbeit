...
In \gls{pws} dynamical systems, bifurcations are possible, that \hl{do not occur} in smooth dynamical systems.
...
\hl{
	Note, that the values of the state space, where the model function is not smooth are called borders.
}
...
\begin{definition}[Rotation Numbers --- Keener]
	...
	Then the rotation number $\rho(\sigma)$ of this cycle is defined as the ratio of \hl{number of letters $\L$ in the symbolic sequence of the cycle divided by the period of the cycle $\sigma$}~\cite{Keener80}.
	\begin{align}
