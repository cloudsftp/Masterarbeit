...
\begin{figure}
	...
	\caption[1D scan of periods associated with parameter regions of a \glsentrylong{pa} structure between two parameter regions that are associated with the fixed points $\L$ and $\R$]{
...
\Cref{fig:state.discont.adding.farey.rot} shows such a Farey-tree showing the rotation numbers of a \gls{pa} structure between two parameter regions associated with the fixed points $\L$ and $\R$.
If we replace the content of the nodes with the symbolic sequences of the cycles associated with the parameter regions in the \gls{pa} structure, we get the tree shown in \Cref{fig:state.discont.adding.farey.sym}.
...
\begin{figure}
	...
	\subfloat[Rotation Numbers]{
		\includegraphics[height=.55 \textheight]{../Figures/2/2.2a/adding.png}
		\label{fig:state.discont.adding.farey.rot}
	}
	\quad
	\subfloat[Symbolic Sequences]{
		\includegraphics[height=.55 \textheight]{../Figures/2/2.2b/adding.png}
		\label{fig:state.discont.adding.farey.sym}
	}
	\caption[Farey-trees of a \glsentrylong{pa} structure between two parameter regions that are associated with the fixed points $\L$ and $\R$]{
...
Another class of bifurcation structures that also occurs in \gls{pws} discontinuous maps is the class of \gls{pi} structures.
As the name suggests, here the periods increment from one parameter region to the next~\cite{granados2017incrementing}.
