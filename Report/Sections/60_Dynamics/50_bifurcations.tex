...
\begin{enumerate}
	...
	\item At the left boundary there will be the bifurcations $\BCB_{d_0}^{\A^a\B^b\C^c\underline{\D}^d}$ and $\BCB_{d_2}^{\A^c\underline{\B}^d\C^a\D^b}$.
...
\todo{
	...
	$\beta$ move branches $f_\A$, $f_\C$ up -> points near borders $d_1$, $d_3$ most movement since a lot of space between bisector and function

	$\beta$ larger -> from left, $\beta$ smaller -> from right


	$\alpha$ moves branches $f_\B$, $f_\D$ ->
}
