...
\Cref{fig:add.change.schema} \hl{shows multiple schematics that are used in this section to summarize all changes.}
\hl{
...
\hl{This is shown in} \Cref{fig:add.change.schema.a}.
\hl{
...
\hl{This is visible in} \Cref{fig:add.change.schema.b}.
\hl{
...
In \Cref{fig:add.change.schema.c}, \hl{this crossing point is not visible anymore, since it moved out of the domain of the picture.}
In \Cref{fig:add.change.schema.d}, \hl{it is visible again, but on the left side of the schema}.
\hl{
...
\hl{This final scenario can be seen in} \Cref{fig:add.change.schema.e}.
\hl{
...
\hl{This point is marked as $C_{P \cap P}^\rightarrow$ in} \Cref{fig:add.change.schema.c}.
\hl{
...
\hl{Both these newly created corner points move right, as can be seen in} \Cref{fig:add.change.schema.d}.
\hl{
...
\hl{The overlapping parameter region $P^m_k \cap P^m_{k+1}$ is now bounded by four boundaries, as can be seen in} \Cref{fig:add.change.schema.e}.
\hl{
...
In \Cref{fig:add.change.schema.c}, \hl{the horizontally neighboring ``type A'' parameter regions overlap and in} \Cref{fig:add.change.schema.d}, \hl{the parameter regions do not overlap}.
\hl{
...
\begin{proof} \phantom{x} \\
	...
	For \Cref{equ:add.change.conseq.sigmaA} to hold, the first point of \hl{the cycle} $\sigma_1$ needs to be left of first point of the cycle $\sigma_2$ on the branch $f_\A$, because \hl{the cycle $\sigma_1$ has one point more on this branch than the cycle $\sigma_2$ and} this branch is increasing.
	...
	For \Cref{equ:add.change.conseq.sigmaB} to hold, \hl{the last point of the cycle $\sigma_2$ must be right of the last point of the cycle} $\sigma_1$ on this branch per the same logic as before.
	...
	Also, \Cref{equ:add.change.conseq.sigmaC} \hl{cannot} be fulfilled if the first point of \hl{the cycle} $\sigma_1$ is left of the first point of \hl{the cycle} $\sigma_2$ on the branch $f_\C$, since \hl{the first point of the cycle with more points, $\sigma_2$ in this case, on the branch must be left of the first point of the other cycle on that branch if the branch is increasing}.
	\hfill $\blacksquare$
\end{proof}
