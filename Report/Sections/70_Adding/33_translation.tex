...
\hl{But we first need to define the notion of syllables which is needed for the} \Cref{def:t,def:T}.
...
\begin{definition}[Translating 4-syllables from \hl{the Halved to the Archetypal Model}]
	\label{def:t}
...
\begin{definition}[Translating Symbolic Sequences from \hl{the Halved to the Archetypal Model}]
	\label{def:T}
...
\hl{The concepts needed for this are defined below in} \Cref{def:shifting,def:shift.equiv}.
...
\hl{The final result of the translation process is a set and, it is defined as the function $F$ which is formally defined in} \Cref{def:F}.
